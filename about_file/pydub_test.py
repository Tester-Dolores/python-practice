...
class TestPydub:
    ...
    def operate(self,resource_path,resource_format,to_format,split_size=0):
        ...
        if split_size != 0:
            ...
            for i, chunk in enumerate(chunks):
                ...
                print(chunk_name)
                chunk.export(chunk_name, format=to_format, parameters=["-ar", "16000","-ac","1", "-acodec", "pcm_s16le"])
        else:
            resource.apply_gain(+10)
            chunk_name = "./temp1/{}_out.wav".format(output_key)
            resource.export(chunk_name,format="wav", parameters=["-ar", "16000","-ac","1", "-acodec", "pcm_s16le"])
            print(chunk_name)
    ...
    def test_pydub(self):
        ...
        """
        self.operate(self.resource_path,self.resource_path.split("\\")[-1].split(".")[1],"wav",split_size=33)
