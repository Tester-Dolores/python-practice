...
for i in range(0,10):
    ...
    if weight == 100: animal = "sheep"
    if weight == 200: animal = "tiger"
    rooms.append({
        "weight":weight,
        "animal":animal
    })
while True:
    ...
    if choose_01 =="a":
        if rooms[random_room]["animal"] == "tiger": print("Wow!! I am tiger\n")
        if rooms[random_room]["animal"] == "sheep": print("mie~~I am sheep\n")
    ...
    if game_ing_time>limit_time: break
    else: print("【Warning】 游戏时间还剩下{}s\n".format(int(limit_time-game_ing_time)))
pprint(rooms)
